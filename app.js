...
let typeNavFilter = null; // null(全部) | water | electric | gas，手機版底部導覽列篩選
let searchKeyword = '';
let saveTimer = null;
...
function renderTable() {
  ...
  ['water', 'electric', 'gas'].forEach((type) => {
    ...
    if (navBtn) {
      navBtn.style.display = columnPrefs[type] ? '' : 'none';
      navBtn.classList.toggle('active', typeNavFilter === type);
    }

    if (!columnPrefs[type]) return;
    if (typeNavFilter && typeNavFilter !== type) return;
...
function setHandlers() {
  ...
  document.querySelectorAll('.type-nav-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      typeNavFilter = typeNavFilter === btn.dataset.target ? null : btn.dataset.target;
      renderTable();
      document.getElementById('cardList').scrollIntoView({ behavior: 'smooth', block: 'start' });
    });
  });
